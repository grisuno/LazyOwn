...
| `do_amass` | Executes Amass to perform a passive enumeration on a given domain. | `lazyown.py` |
| `do_arjun` | Executes an Arjun scan on the specified URL for parameter discovery. | `lazyown.py` |
| `do_arpscan` | Executes an ARP scan using `arp-scan`. | `lazyown.py` |
| `do_batchnmap` | Runs the internal module `modules/lazynmap.sh` for multiple Nmap scans. | `cli/commands/recon.py` |
| `do_bbot` | Executes a BBOT scan to perform various reconnaissance tasks. | `lazyown.py` |
...
| `do_dirsearch` | Runs the `dirsearch` tool to perform directory and file enumeration on a specified URL. | `lazyown.py` |
| `do_dmitry` | This function constructs and executes a command for the 'dmitry' tool. | `lazyown.py` |
...
| `do_feroxbuster` | Command feroxbuster: Installs and runs Feroxbuster for performing forced browsing and directory brute-forcing. | `lazyown.py` |
...
| `do_gobuster` | Uses `gobuster` for directory and virtual host fuzzing based on provided parameters. Supports directory enumeration and virtual host discovery. | `lazyown.py` |
| `do_hostdiscover` | Discover active hosts in a subnet by performing a ping sweep. | `lazyown.py` |
...
| `do_magicrecon` | Command magicrecon: Automates the setup and usage of MagicRecon to perform various types of reconnaissance and vulnerability scanning on specified targets. | `lazyown.py` |
...
| `do_nmapscript` | Perform an Nmap scan using a specified script and port. | `lazyown.py` |
| `do_nuclei` | Executes a Nuclei scan on a specified target URL or host. | `lazyown.py` |
...
| `do_osmedeus` | Executes Osmedeus scans with guided input for various scanning scenarios. | `lazyown.py` |
...
| `do_portdiscover` | Scan all ports on a specified host to identify open ports. | `lazyown.py` |
| `do_portservicediscover` | Scan all ports on a specified host to identify open ports and associated services. | `lazyown.py` |
...
| `do_skipfish` | This function executes the web security scanning tool Skipfish | `lazyown.py` |
...
| `do_vscan` | Perform port scanning using vscan with the provided parameters. | `lazyown.py` |
