...
Covers the ``do_ask``, ``do_groq``, ``do_ai_playbook`` and ``do_ai_toggle``
commands. ``ask`` answers through the configured ``llm_backend`` (any
provider); ``groq`` pins the Groq backend explicitly. Both resolve models,
keys, and budget through :mod:`modules.llm_factory` — no subprocess, no
...
class AiCommandSet(LazyOwnCommandSet):
    ...
    def do_ask(self, line):
        ...
        print_msg(ask_general(full_prompt))
    ...
    def do_groq(self, line):
        ...
        full_prompt = truncate_message(config.render("oneliner", prompt))
        print_msg(f"Asking groq model {model}...")
        try:
            print_msg(backend.complete(DEFAULT_SYSTEM_PROMPT, full_prompt))
        except Exception as exc:
            print_error(f"groq request failed: {exc}")
