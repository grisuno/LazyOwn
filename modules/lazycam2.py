...
def list_interfaces():
    command = "ip a show scope global | awk '/^[0-9]+:/ { sub(/:/,\"\",$2); iface=$2 } /^[[:space:]]*inet / { split($2, a, \"/\"); print iface\" \"a[1]\" \"a[2] }'"
...
def get_subnet_from_interface(interface):
    ip, mask = interface[1], interface[2]
    return f"{ip}/{mask}"

def get_ip_addresses(interface):
    # Ejecuta el comando arp-scan para escanear la red
    command = f"sudo arp-scan -I {interface} --localnet"
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    output = result.stdout.strip()

    # Extrae las direcciones IP de la salida
    ip_addresses = []
    lines = output.split('\n')
    for line in lines:
        parts = line.split()
...
iface = choose_interface(show_ifaces)
iface_name = iface[0]

# Obtener las IPs de la red usando arp-scan
ip_addresses = get_ip_addresses(iface_name)
print("IPs encontradas en la red:")
...
if args.ip:
    ...
    if ip_addresses:
        filter_str = " or ".join([f"host {ip}" for ip in ip_addresses])
    else:
        subnet = get_subnet_from_interface(iface)
        filter_str = None  # No aplicar filtro en scapy, se filtrará manualmente
...
def handle_packet(packet):
    ...
    if IP in packet and Raw in packet:
        ...
        # Aplicar filtro manualmente si se captura toda la red
        if args.all:
...
def run():
    if args.verbose:
        print(f"Capturando paquetes{' con filtro: ' + filter_str if filter_str else ''}...")
