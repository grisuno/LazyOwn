...
def write_utils_md(
    ...
    with open(output_path, "w", encoding="utf-8") as fh:
        fh.write("# LazyOwn Utils Reference\n\n")
        fh.write("Auto-generated by readmeneitor.py from helper docstrings ")
        fh.write("in utils.py.\n\n")

        fh.write("## Table of Contents\n\n")
        for name in sorted(functions):
            anchor = name.lower().replace("_", "-")
            fh.write(f"- [`{name}`](#{anchor})\n")
        fh.write("\n---\n\n")

        for name in sorted(functions):
            docstring = functions[name]
            fh.write(f"### `{name}`\n\n")
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} lazyown.py|utils.py")
        print("  lazyown.py generates COMMANDS.md by scanning lazyown.py and cli/commands/")
        print("  utils.py generates UTILS.md by scanning helper functions in utils.py")
        print("  Requires cli/command_index.json for command metadata.")
        sys.exit(1)

    target_arg = sys.argv[1]
    if target_arg == "utils.py":
        functions = extract_functions_from_file(UTILS_SOURCE)
        if not functions:
            print("[!] No helper functions found. Aborting.")
            sys.exit(1)
        total = write_utils_md(functions, UTILS_OUTPUT)
        print(f"[+] Done. {total} helpers documented in {UTILS_OUTPUT}")
        return
