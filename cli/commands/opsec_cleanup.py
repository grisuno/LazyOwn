...
class OpsecCleanupCommandSet(LazyOwnCommandSet):
    ...
    def do_opsec_score(self, line: str) -> None:
        ...
        self._cmd.poutput(
            f"    Detection       : {', '.join(assessment.detection_surface) if assessment.detection_surface else 'none'}"
        )
    ...
    def do_log_tamper(self, line: str) -> None:
        ...
        if platform == "all":
            ...
            for plat, data in all_cmds.items():
                cmds = data.get(
                    f"{plat}_commands",
                    data.get("powershell_commands", data.get("bash_commands", data.get("zsh_commands", []))),
                )
        ...
        cmds = result.get(
            ...
            result.get(
                "powershell_commands",
                result.get("bash_commands", result.get("zsh_commands", result.get("commands", []))),
            ),
        )
    ...
    def do_timestomp(self, line: str) -> None:
        ...
        """
        from modules.timestomper import TimestompConfig, Timestomper
    ...
    def do_network_opsec(self, line: str) -> None:
        ...
        """
        from modules.network_opsec import NetworkOpsecConfig, NetworkOpsecEngine
