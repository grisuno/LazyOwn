...
| `tests/` | 129 files, ~2050 tests. No mocking of C2 or daemon. |
...
| Contract file | One-line contract | Tests / mutation gate |
|---|---|---|
| `modules/killchain.py` | canonical phases, snapshot, current_phase | `test_killchain_unified_v2.py`; `run_mutation_killchain.py` 4/4 |
| `modules/world_model.py` | engagement state, email/domain entries, policy facts | `test_killchain_snapshot.py` |
| `modules/beacon_history.py` | per-implant command history (JSONL) | `test_beacon_history.py` |
| `cli/tips_engine.py` | unified post-command tips; killchain auto-display; phase via `KillChain`; rich rendering; `killchain_display` config field | `test_tips_engine.py`, `test_killchain_auto_refresh.py` |
| `cli/reactive_hints.py` | evidence hints with confidence in `[0, 99]`; shared `read_run_commands`; DRY `_collect_command_hints` | `test_evidence_hints.py` |
| `cli/recommendation_signals.py` | gap/topology/playbook signals; shared `_load_world_model`; named constants | `test_killchain_gap_signal.py` |
| `cli/chain_mode.py` | interactive chain prompt; re-prompt on invalid pick; named key constants; logged atomic store | `test_chain_mode.py` |
| `cli/noise_verbs.py` | single source of truth for the skip-verb lists (base + per-surface extras) | `test_tips_engine.py`, `test_chain_mode.py` |
| `core/api_authz.py` | tenant-bound API keys; rotation grace implemented; permissions copied from rotated key; JSON 401/403 | `test_api_authz.py` (34); `run_mutation_api_authz.py` 7/7 |
| `core/logging.py` | structured JSON-lines; `install_json_handler` preserves existing handlers, idempotent | `test_structured_logging.py` (13) |
| `lazyc2/addon_creator.py` | addon YAML contract; secure atomic writes (mkstemp + fchmod + fsync + replace) | `test_addon_creator.py` (71); `run_mutation_addon_creator.py` 9/0 |
| `lazyc2/blueprints/api.py` | health endpoints; `/api/health/tenant` actually enforced via `require_api_auth_with_store` | `test_api_authz.py` |
| `cli/engagement_hooks.py` | ELO sync bound to module `USERS_PATH`; host-login-independent tests | `test_engagement_elo_and_methodology.py` (45) |
