...
if [[ -f "$DEST_FILE" ]]; then
    ...
    "El archivo $DEST_FILE ya existe. borrando..."
	rm -rf "$DEST_FILE"
fi
...
if command -v wget &>/dev/null; then
    ...
    --foreground 212 --border-foreground 212 --border double \
    --align center --width 50 --margin "1 2" --padding "2 4" \
    "wget encontrado. Descargando con wget..."
	download_with_wget
elif command -v curl &>/dev/null; then
    gum style \
    --foreground 212 --border-foreground 212 --border double \
    --align center --width 50 --margin "1 2" --padding "2 4" \
    "wget no encontrado. Descargando con curl..."
	download_with_curl
else
    gum style \
    --foreground 212 --border-foreground 212 --border double \
    --align center --width 50 --margin "1 2" --padding "2 4" \
    "Ni wget ni curl están instalados. Por favor, instala uno de ellos para continuar."
	exit 1
fi
