...
#-- Target
p_ip_nmap5 = r'Interesting.*on\s(?:(?P<fqdn_nmap5>.*) (?=\((?P<ip_nmap5>{})\)))|Interesting.*on\s(?P<ip_only_nmap5>.*)\:'.format(p_ip_elementary)
p_ip_nmap6 = r'Nmap.*for\s(?:(?P<fqdn_nmap6>.*) (?=\((?P<ip_nmap6>{})\)))|Nmap.*for\s(?P<ip_only_nmap6>{})$'.format(p_ip_elementary, p_ip_elementary)

p_ip = re.compile('{}|{}'.format(p_ip_nmap5, p_ip_nmap6))

#-- rDNS
p_rdns = re.compile(r'rDNS record for (?P<ip>{}):\s(?P<rdns>.*)$'.format(p_ip_elementary))
...
#-- MAC address
p_mac = re.compile(r'MAC Address:\s(?P<mac_addr>({}))\s\((?P<mac_vendor>.*)\)'.format(p_mac_elementary))
...
def split_grepable_match(raw_string):
    ...
    # Patterns
    p_host = re.compile(r'Host:\s(?P<ip>{})\s+\((?P<fqdn>|.*)\)'.format(p_ip_elementary))
...
def parse(fd):
    ...
    for line in lines:
        ...
        if port and last_host is not None:
...
def parse_xml(xml_file):
    ...
    try:
        ...
        print("[!] An error has occurred while parsing the XML file: '{}'.\nExiting".format(e))
        return None
        ...
        if 'up' in host.find('status').get('state'):
            ...
            for port in open_ports:
                ...
                if service is not None:
                    ...
                    version = ("{} {} {}".format(service_product, service_version, service_extrainfo)).strip()
                ...
                for script in scripts:
                    script_output = script_output + "\n{}: {}".format(script.get('id'), script.get('output'))
...
def main():
    ...
    if not valid_format:
        parser.error("Please specify a valid output format: '{}' is invalid \n\
         Supported objects are {{ fqdn, rdns, hop_number, ip, mac_address, mac_vendor, port, protocol, os, script, service, version }}".format(', '.join(unknown_items)))

    # Input selection
    if (options.input is not None) and (options.xml_input is not None):
        parser.error("Please specify either a normal/grepable or an XML input file")

    elif (options.input is None) and (options.xml_input is not None):
        results = parse_xml(options.xml_input)

    elif options.xml_input is None:
